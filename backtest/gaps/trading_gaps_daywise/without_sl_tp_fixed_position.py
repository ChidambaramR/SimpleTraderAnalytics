...
class GapTraderFixedPosition(DayTrader):
    ...
    def generate_trades(self, stock, day_data, minute_data):
        ...
        available_capital = self.current_equity * 5  # 5x leverage
        per_stock_amount = available_capital / min(len(self.daily_gaps), self.top_n)
        
        # Get current stock's gap info
        gap = next(gap for gap in self.daily_gaps if gap['symbol'] == stock)
        
        # Calculate quantity
        quantity = floor(per_stock_amount / gap['open'])
        ...
        trade = {
            ...
            'Invested amount': per_stock_amount,
            'Quantity': quantity
        }
